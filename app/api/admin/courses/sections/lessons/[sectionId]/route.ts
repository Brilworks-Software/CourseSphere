...
export async function POST(
  ...
  if (section.course_id !== course_id) {
    return NextResponse.json(
      { error: "Section does not belong to the specified course" },
      { status: 400 },
    );
  }
...
export async function PATCH(
  ...
  let moveToSectionId: string | undefined = undefined;
  ...
  try {
    ...
    title = body.title;
    moveToSectionId = body.moveToSectionId;
  } catch {
    // No body, treat sectionId as lessonId (for PATCH /lessons/[lessonId])
    lessonId = sectionId;
  }
  ...
  if (moveToSectionId) {
    ...
    const { data, error } = await supabase
      ...
      .update({ title })
      .eq("id", lessonId)
      // .eq('section_id', sectionId) // Optionally keep this if you want to restrict update to section
      .select()
      .single();
    if (error)
      return NextResponse.json({ error: error.message }, { status: 500 });
