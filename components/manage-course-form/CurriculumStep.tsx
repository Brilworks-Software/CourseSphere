...
export function CurriculumStep({ courseId }: CurriculumStepProps) {
  ...
  async function fetchSection(sectionId: string) {
    try {
      ...
      setSections(prev =>
        prev.map(sec => (sec.id === sectionId ? data : sec))
      );
    } catch (e: any) {
      setError(e.message);
    }
  ...
  async function handleAddLesson(e: React.FormEvent, sectionId: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/courses/sections/lessons/${sectionId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleEditLesson(sectionId: string, lessonId: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/courses/sections/lessons/${lessonId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
          ...
          {[...Array(2)].map((_, idx) => (
            <div key={idx} className="border rounded-lg p-4 mb-4 bg-muted">
              ...
              <div className="space-y-2 ml-4">
                {[...Array(2)].map((_, lidx) => (
                  <div key={lidx} className="flex items-center border rounded p-2 bg-background">
                  ...
                  <div className={`flex items-center justify-between w-full px-4 py-3 transition-all `}>
                    <AccordionTrigger className={`flex-1 min-w-0 py-0 w-full  hover:no-underline`}>
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {section.description && (
                          <span className="ml-2 text-xs text-muted-foreground italic truncate">{section.description}</span>
                        )}
                      </div>
                    </AccordionTrigger>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        <DropdownMenuItem onClick={() => { setEditingSectionId(section.id); setSectionTitleEdit(section.title); }}>
                  ...
                  <AccordionContent >
                    ...
                    {editingSectionId === section.id && (
                      <form onSubmit={e => { e.preventDefault(); handleEditSection(section.id); }} className="flex gap-2 mx-2">
                        <Input value={sectionTitleEdit} onChange={e => setSectionTitleEdit(e.target.value)} className="w-full" required minLength={3} />
                        <Button type="submit" size="sm">Save</Button>
                        <Button type="button" size="sm" variant="outline" onClick={() => setEditingSectionId(null)}>Cancel</Button>
                      </form>
                    )}
                    {/* Lessons List */}
                    <div className="space-y-2 p-3">
                      {section.lessons.map((lesson) => (
                        ...
                        >
                          <div className="flex items-center w-full">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent>
                                ...
                                <DropdownMenuItem onClick={() => handleDeleteLesson(section.id, lesson.id)} className="text-destructive">
                          ...
                          {editingLessonId === lesson.id && (
                            <form onSubmit={e => { e.preventDefault(); handleEditLesson(section.id, lesson.id); }} className="flex gap-2 w-full mt-2">
                              <Input  value={lessonTitleEdit} onChange={e => setLessonTitleEdit(e.target.value)} minLength={3} required />
                              <Button type="submit" size="sm">Save</Button>
                              <Button type="button" size="sm" variant="outline" onClick={() => setEditingLessonId(null)}>Cancel</Button>
                            </form>
                          )}
                      ...
                      {showLessonDialog === section.id ? (
                        <form onSubmit={e => handleAddLesson(e, section.id)} className="flex gap-2 mt-2">
                          <Input value={newLessonTitle} onChange={e => setNewLessonTitle(e.target.value)} placeholder="New Lecture Title" required minLength={3}  />
                          <Button type="submit" disabled={loading}><Plus className="w-4 h-4 mr-1" /> Add Lecture</Button>
                          <Button type="button" variant="outline" onClick={() => setShowLessonDialog(null)}>Cancel</Button>
                        </form>
                      ) : (
                        <Button
                          // variant="secondary"
          ...
          <Dialog open={showSectionDialog} onOpenChange={setShowSectionDialog}>
            <DialogContent>
              ...
              <form onSubmit={handleAddSection} className="space-y-2">
                <Input value={newSectionTitle} onChange={e => setNewSectionTitle(e.target.value)} placeholder="Enter a Title" required minLength={3} maxLength={80} />
                <div>
                  <label className="text-sm font-medium">What will students be able to do at the end of this section?</label>
                  <Input value={newSectionDesc} onChange={e => setNewSectionDesc(e.target.value)} placeholder="Enter a Learning Objective" maxLength={200} />
                </div>
                <DialogFooter>
                  <Button type="submit" disabled={loading}>Add Section</Button>
